...
class AreaUnderIntegral(GraphScene):
    CONFIG = {
        ...
        "y_min": 0,
        "y_max": 6,
        "Func": lambda x: 1 + x ** 2 * np.exp(-0.15 * x ** 2),
    }
    ...
    def construct(self):
        ...
        int_area_sym = TextMobject(
            "$$\int_{a}^b f(x)dx$$"
        ).shift(2 * UP)
        area_mean_text = TextMobject(
            r"means area under the curve of $f(x)$ \\ in the region $a\leq x\leq b$"
        ).next_to(int_area_sym, DOWN)
        ...
        min_lim = self.get_vertical_line_to_graph(
            1, func, DashedLine, color=YELLOW
        )
        tick_a = TextMobject(r"$a$").next_to(min_lim, DOWN)
        max_lim = self.get_vertical_line_to_graph(
            4, func, DashedLine, color=YELLOW
        )
        ...
        self.wait(2)
        self.play(
            ShowCreation(min_lim),
            Write(tick_a),
            ShowCreation(max_lim),
            Write(tick_b),
            run_time=0.5,
        )

        approx_text = TextMobject(
            r"The area can be approximated as \\ sum of small rectangles"
        ).next_to(func, 4 * Y)
        ...
        rects = self.get_riemann_rectangles(
            flat_graph, x_min=1, x_max=4, dx=0.5
        )
        for new_rects in rect_list:
            ...
            self.play(
                Transform(
                    rects, new_rects, run_time=1.5, lag_ratio=0.5
                )
            )
            
        conclude_text = TextMobject(
            r"Making the rectangles infinitesimally thin \\ we get the real area under the curve."
        ).next_to(func, 4 * Y)

...
class YlimitXdependent(GraphScene):
    ...
    def construct(self):
        ...
        line = self.get_graph(
            lambda x: 2 - 2 * x, x_min=0, x_max=1, color=RED
        )
        line_eqn = (
            TextMobject("2x+y=2")
            .move_to(self.graph_origin + 0.8 * X + Y)
            .rotate(np.arctan(-2))
        )
        self.line = line

        caption = TextMobject(
            r"The value of $y$ is\\ changing with $x$"
        ).move_to(self.graph_origin + 1.2 * X + 1.8 * Y)
    ...
    def show_y_values_at_different_x(self):
        ...
        for rect in rects_subset:
            ...
            if last_rect is not None:
                ...
                anims += [GrowFromCenter(brace), Write(y)]
            self.play(*anims)
            # self.wait(.2)

...
class ElementaryArea(GraphScene):
    ...
    def construct(self):
        ...
        caption = TextMobject("The elementary area in ").to_edge(
            UP
        )
        rect_text = TextMobject("Cartesian Coordinates").next_to(
            caption,
            DOWN,
        )
        polar_text = TextMobject("Polar Coordinates").next_to(
            caption,
            DOWN,
        )
        ...
        self.play(
            ReplacementTransform(rect_text, polar_text),
...
#=============================================================
    ...
    def get_rect_element(self):
        ...
        dydx = TextMobject("$dxdy$", color=BLACK).next_to(
            rect, IN
        )
    ...
    def get_polar_element(self):
        ...
        # # #getting braces
        r_in_theta1 = self.graph_origin + r_in * (
            np.cos(theta1) * RIGHT + np.sin(theta1) * UP
        )
        dr_line = Line(
            r_in_theta1, r_in_theta1 + RIGHT * (r_out - r_in)
        )
        dr_brace = Brace(dr_line, DOWN, buff=SMALL_BUFF).rotate(
            theta1, about_point=r_in_theta1
        )
        ...
        rdt_line = Line(
            r_out_theta2, r_out_theta2 + DOWN * (r_out * dtheta)
        )
        rdt_brace = Brace(
            rdt_line, RIGHT, buff=MED_SMALL_BUFF
        ).rotate(theta2 - (dtheta / 2), about_point=r_out_theta2)
        rdt_label = rdt_brace.get_text(
            "$rd\\theta$", buff=SMALL_BUFF
        )
        rdt_brace_gr = VGroup(rdt_brace, rdt_label)

        # getting label r and dtheta
        r1 = DashedLine(
            self.graph_origin, r_in_theta1
        ).set_color(RED)
        r2 = DashedLine(
            self.graph_origin, r_out_theta2
        ).set_color(RED)
        r_brace = Brace(r1, DOWN, buff=SMALL_BUFF).rotate(
            theta1, about_point=self.graph_origin
        )
        ...
        dtheta_arc_label = TextMobject("$d\\theta$").move_to(
            0.99 * dtheta_arc.get_corner(UR)
        )
        dtheta_label = VGroup(dtheta_arc, dtheta_arc_label)

        rdrdt = TextMobject(
            "$rdrd\\theta$", color=BLACK
        ).next_to(arc, IN)

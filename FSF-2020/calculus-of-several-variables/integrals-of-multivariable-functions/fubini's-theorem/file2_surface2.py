...
class SurfacesAnimation(ThreeDScene):
    ...
    def construct(self):
        ...
        fn_text = (
            TextMobject("$z=xy$").set_color(BLUE).scale(1.5)
        )
        ...
        # get boundary curves
        c1 = self.get_curve(self.axes, lambda x: x ** 2 / 4)
        c1_label = (
            TextMobject("$y=x^2$")
            .next_to(c1, IN + OUT)
            .shift(DOWN + RIGHT)
        ...
        c2 = self.get_curve(self.axes, lambda x: x).set_color(
            PINK
        )
        c2_label = TextMobject("$y=x$").next_to(c2, IN + OUT)
        c2_label.rotate(
            PI / 2, about_point=(c2_label.get_corner(UL))
        )
    ...
    def get_curve(self, axes, func, **kwargs):
        ...
        return ParametricFunction(
            lambda x: axes.c2p(x, func(x), 0), **config
        )

    def get_region(self, axes, curve1, curve2, **kwargs):
        x_vals = np.arange(axes.x_min, axes.x_max, 0.1)
        c1_points = [

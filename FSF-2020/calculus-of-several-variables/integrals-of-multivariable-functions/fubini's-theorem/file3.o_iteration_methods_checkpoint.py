...
class IterationMethods(GraphScene):
    ...
    def construct(self):
        ...
        caption_y_int = TextMobject(
            r"Observe the limits\\ of  integration"
        ).to_corner(UR)
        int_lim = (
            TextMobject("$$\\int_0^1$$")
            .next_to(caption_y_int, DOWN, buff=0.5)
            .align_to(caption_y_int, LEFT)
        )
        ...
        self.play(
            ApplyMethod(
                self.y_int.next_to, int_lim, RIGHT, buff=0
            )
        )

        self.play(
            ApplyMethod(self.dx_label.next_to, self.y_int, RIGHT)
        )

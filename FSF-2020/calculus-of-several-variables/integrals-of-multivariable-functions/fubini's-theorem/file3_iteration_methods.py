...
class IterationMethods(GraphScene):
    ...
    def construct(self):
        ...
        curve1 = self.get_graph(
            lambda x: x ** 2, x_min=0, x_max=1, color=ORANGE
        )
        ...
        curve2 = self.get_graph(
            lambda x: x, x_min=0, x_max=1, color=YELLOW
        )
        ...
        caption_limit = TextMobject(
            r"Observe the limits\\ of  integration"
        ).to_corner(UR)
        int_lim = (
            TextMobject("$$\\int_0^1$$")
            .next_to(caption_limit, DOWN, buff=0.5)
            .align_to(caption_limit, LEFT)
        )
        self.int_lim = int_lim
        self.play(
            ShowCreation(VGroup(curve1, curve2)),
            Write(VGroup(c2_eqn, c1_eqn)),
        )
    ...
    def intro_scene(self):
        text = TextMobject(
            r"How different orders of \textbf{iterated  integral}\\ works over the same region ?"
        )
    ...
    def integral_setup(self, ref_object, first_y=True):
        if first_y:
            ...
            self.play(
                ApplyMethod(
                    self.y_int.next_to, ref_object, RIGHT, buff=0
                )
            )

            self.play(
                ApplyMethod(
                    self.dx_label.next_to, self.y_int, RIGHT
                ),
                ShowCreation(area),
                Write(self.int_lim),
                run_time=4,
            ...
            self.play(
                ApplyMethod(
                    self.x_int.next_to, ref_object, RIGHT, buff=0
                )
            )
            self.play(
                ApplyMethod(
                    self.dy_label.next_to, self.x_int, RIGHT
                ),
                ShowCreation(area),
                Write(self.int_lim_y),
                run_time=4,
            )
    ...
    def show_integral_values_at_different_x(self):
        ...
        for rect in rects_subset:
            brace = Brace(rect, LEFT, buff=0.1)
            y_int = TexMobject(
                "\\int_{x^2}^{x}dy"
            )  # .rotate(PI/2)
            ...
            if last_rect is not None:
                ...
                anims += [GrowFromCenter(brace), Write(y_int)]
            self.play(*anims)
            # self.wait(.2)
    ...
    def show_integral_values_at_different_y(self):
        ...
        for rect in rects_subset:
            ...
            x_int = TexMobject("\\int_{y}^{\sqrt y}dx").rotate(
                -PI / 2
            )
            ...
            if last_rect is not None:
                ...
                anims += [GrowFromCenter(brace), Write(x_int)]
            self.play(*anims)
            # self.wait(.2)
    ...
    def bounded_riemann_rectangles(
        ...
        for x, color in zip(x_range, colors):
            ...
            graph1_point = self.input_to_graph_point(
                sample_input, graph1
            )
            graph1_point_dx = self.input_to_graph_point(
                sample_input + width_scale_factor * dx, graph1
            )
            graph2_point = self.input_to_graph_point(
                sample_input, graph2
            )
            ...
            if (
                graph1_point[1] < self.graph_origin[1]
    ...
    def bounded_riemann_rectangles_y(
        ...
        colors = color_gradient(
            [start_color, end_color], len(y_range)
        )

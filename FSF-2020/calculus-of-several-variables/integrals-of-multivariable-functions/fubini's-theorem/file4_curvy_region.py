...
class CurvyRegion(GraphScene):
    ...
    def construct(self):
        ...
        sin_curve_points = [
            self.graph_origin + (2 + 0.5 * np.sin(2 * y), y, 0)
            for y in np.arange(1, 5, 0.005)
        ]

        cos_curve_points = [
            self.graph_origin + (5 + 0.5 * np.cos(2 * y), y, 0)
            for y in np.arange(1, 5, 0.005)
        ]
    ...
    def first_y_int_scene(self):
        talk = TextMobject(
            r"For doing the $y$ integration\\ first we need to set\\ proper $y$ limts"
        ).to_corner(UR, buff=LARGE_BUFF)
        problem = TextMobject(
            r"But here we get\\ more than two $y$ values\\ for a single $x$ value"
        ).to_corner(UR, buff=LARGE_BUFF)
        int_y = TextMobject("$$\\int_?^? dy$$").next_to(
            problem, DOWN, buff=0.5
        )
    ...
    def try_x_first_scene(self):
        try_text = TextMobject(
            r"But if we try to integrate\\ along $x$ first ...."
        ).to_corner(UR, buff=LARGE_BUFF)
        good_limits = TextMobject(
            r"For one $y$ value we get\\ only \textbf{two} $x$ values $\dots$"
        ).to_corner(UR, buff=LARGE_BUFF)
        limit_values = TextMobject(
            r"one Lower limit\\ one Upper limit "
        ).next_to(good_limits, DOWN, buff=0.5)
        int_x = TextMobject(
            "$$\\int_{f(y)}^{g(y)} dx$$"
        ).next_to(limit_values, DOWN)
        ...
        self.wait()
        self.play(
            ApplyMethod(self.line.shift, 3 * self.Y), run_time=4
        )
    ...
    def setup_line(self):
        ...
        line.move_to(
            self.graph_origin + 0.5 * self.X + 1.5 * self.Y, LEFT
        )
        self.line = line

...
class GradOfScalarFunc(ThreeDScene):
    ...
    def construct(self):
        ...
        scalar_fn_text = TexMobject("f(x,y)=", "xy").set_color(
            BLUE
        )
        ...
        grad_text = TexMobject(
            r"\dfrac{\partial f}{\partial x} \hat i+\dfrac{\partial f}{\partial y} \hat j"
        ).set_color(GOLD)
        grad_text.next_to(scalar_fn_text, DOWN).scale(0.9)

        VGroup(grad_text[0][1], grad_text[0][9]).set_color(BLUE)
        VGroup(grad_text[0][5:8], grad_text[0][13:16]).set_color(
            WHITE
        )

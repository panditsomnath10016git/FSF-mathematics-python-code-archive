...
class LineIntegration(GraphScene):
    CONFIG = {
        ...
        "default_vector_field_config": {
            ...
            "opacity": 0.75,
            "vector_config": {"stroke_width": 1.5},
        },
        "a": 0.45,
        "b": 2,
        "path_color": PURPLE,
    ...
    def construct(self):
        ...
        vector_field_text = TexMobject(
            "\\vec F(x,y)", "=y\hat i+x\hat j", stroke_width=1.5
        ).to_edge(TOP, buff=0.2)

        vector_field_text[0][0:2].set_color(TEAL)

        grad_f = TexMobject(
            "\\vec\\nabla f(x,y)", stroke_width=1.5
        )
        ...
        self.wait()
        self.play(
            ReplacementTransform(vector_field_text[0], grad_f)
        )
        self.get_endpoints_of_curve()
        self.wait(0.6)
        vector_field.set_fill(opacity=0.4)
    ...
    def get_endpoints_of_curve(self):
        ...
        for point, label in zip(points, point_labels):
            dot = Dot(self.coords_to_point(*point)).set_color(
                RED
            )
            dot_label = TexMobject(label)
            dot_label.next_to(dot, DR, buff=0.2)
    ...
    def show_line_integral(self):
        ...
        int_value = TexMobject(
            r"=f(P_i)-f(P_f)", stroke_width=1.5
        ).next_to(int_text, DOWN)
        VGroup(int_value[0][1], int_value[0][7]).set_color(
            LIGHT_BROWN
        )

        path_indepent_text = TextMobject(
            r"Value of the Line Integral is\\ independent of Path",
            color=GOLD,
            stroke_width=2,
        ).to_corner(DR, buff=1)
        ...
        self.play(Write(VGroup(int_text, int_value)), run_time=2)
        self.wait(1.5)
    ...
    def show_path(self, points):
        points = (
            [self.end_points[0]] + points + [self.end_points[1]]
        )

        path = VMobject()
        path.set_points_smoothly(
            [self.coords_to_point(*point) for point in points]
        )

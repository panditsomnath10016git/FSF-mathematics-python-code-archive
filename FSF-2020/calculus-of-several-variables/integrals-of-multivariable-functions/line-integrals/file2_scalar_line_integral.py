...
class LineIntegrationProcess(SpecialThreeDScene):
    ...
    def construct(self):
        ...
        fn_text.to_corner(UR, buff=0.8).shift(DOWN)

        # get the surface
        surface = self.get_surface(lambda x, y: self.Func(x, y))
        surface.set_style(
            fill_opacity=0.5,
            fill_color=BLUE_D,
            stroke_width=0.5,
    ...
    def get_line_of_int(self):
        line_of_int_text = TextMobject(
            ...
            "$\\vec r(t)=\cos(t)\hat x+\sin(t)\hat y$",
        )
        line_of_int_text[1].set_color(PINK)
        line_of_int_text.to_edge(TOP, buff=SMALL_BUFF)

        line_of_int = self.get_curve(self.Func, on_surface=False)
    ...
    def get_field_values_on_line(self):
        ...
        values_on_line_text = TextMobject(
            "Values", " of", " function", "on the ", "line"
        )
    ...
    def trasform_to_graphs(self):
        ...
        line_graph = self.get_graph(self.Func, on_surface=False)
        ...
        into_graph = [
            ReplacementTransform(
                self.values_on_surface, on_surface_graph
            ),
            ReplacementTransform(self.line_of_int, line_graph),
            ReplacementTransform(self.area, graph_area),
        ]
    ...
    def get_lines(self):
        ...
        axes = self.axes
        labels = [
            axes.x_axis.n2p(axes.a),
            axes.x_axis.n2p(axes.b),
            axes.y_axis.n2p(axes.c),
    ...
    def get_three_d_axes(
        ...
        # Add xy-plane
        input_plane = self.get_surface(lambda x, t: 0)

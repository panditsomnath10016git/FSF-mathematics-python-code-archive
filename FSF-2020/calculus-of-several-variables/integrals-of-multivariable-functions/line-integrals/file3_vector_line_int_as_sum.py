...
class LineIntegrationAsSum(GraphScene):
    ...
    def write_about_breaking(self):
        breaking_text = TextMobject(
            "\\texttt{..broken}", " into small", "subarcs"
        )
    ...
    def get_breakers(self, dl):
        ...
        while point < (self.b - dl):
            ...
            breaker.rotate(PI / 2).scale(0.5)

            point = end
            self.play(FadeIn(breaker), run_time=0.2)
        #  self.add(breaker)
    ...
    def show_the_sum(self):
        at_any_points_text = TextMobject(
            "At any ", "point", "in each ", "subarc"
        )
        ...
        evaluate_text = TextMobject(
            "$\\vec F(x,y)$ ", "is evaluated"
        ).next_to(at_any_points_text, DOWN)
        evaluate_text.set_color_by_tex("$\\vec F(x,y)$", ORANGE)

        multiply_text = TextMobject(
            "...is multiplied with ", "$\\Delta s_i$"
        )
        ...
        for point in self.points:
            ...
            dot = Dot(
                point=self.input_to_graph_point(
                    point, self.curve
                ),
                radius=0.7 * DEFAULT_DOT_RADIUS,
                stroke_width=0,
                fill_opacity=1.0,
                color=YELLOW,
            )
    ...
    def show_the_dot_product(self):
        ...
        point_coord.next_to(dot, DL, buff=0.01).scale(0.8)

        func_val = TextMobject(
            "$\\vec F(x_i,y_i)$", color=ORANGE
        )
        func_val.next_to(dot, UR).scale(0.8)
    ...
    def show_specific_vectors(self, dots):
        for dot in dots:
            vector = self.vector_field.get_vector(
                dot.get_center()
            )
            vector.set_color(ORANGE)

            self.play(Write(vector), run_time=0.2)

    def show_tangent(self, dot):
        tangent_sym = TextMobject(
            "$\\vec T(x_i,y_i)$", color="#DC75CD"
        ).scale(0.8)
        x = dot.get_center()
        angle = self.angle_of_tangent(
            self.point_to_coords(x)[0], self.curve, dx=0.01
        )
        vect = Vector().rotate(angle, about_point=x)
        vect.set_color("#DC75CD")
        tangent = vect.next_to(x, DR, buff=0)
        tangent_sym.next_to(tangent, DOWN, buff=0.1)
    ...
    def dot_product(self):
        ...
        self.play(
            FadeOut(
                VGroup(
                    self.at_any_points_text, self.evaluate_text
                )
            )
        )
        ...
        self.play(
            FadeIn(dot_sym),
            ApplyMethod(
                self.tangent_sym.next_to, dot_sym, RIGHT
            ),
        )
    ...
    def get_ds(self):
        ...
        ds_brace_label = ds_brace.get_text(
            "$\Delta s_i$", buff=0.05
        )
    ...
    def construct_equation(self):
        sum_up_text = TextMobject(
            "and", " summed ", "for all i' s"
        )
        sum_up_text.set_color_by_tex("summed", PURPLE_A)
        sum_up_text.next_to(
            self.multiply_text, DOWN, buff=MED_SMALL_BUFF
        )
        sum_up_text.shift(LEFT)

        sum_eqn = TextMobject("$$\\sum_i^{ } $$").set_color(
            PURPLE_A
        )
        sum_eqn.move_to(
            self.graph_origin + 6.5 * self.X + 4 * self.Y
        )

        line_integral_text = TextMobject(
            "The Value of the", " line ", "integral is"
        ).to_edge(TOP, buff=MED_SMALL_BUFF)
        line_integral_text.set_color_by_tex("line", BLUE_C)
        approx = TextMobject("$\\approx$", color=RED).next_to(
            sum_eqn, LEFT
        )
        multipled = VGroup(
            self.func_val,
        ...
        self.play(ApplyMethod(multipled.next_to, sum_eqn, RIGHT))
        self.wait()
        self.play(
            FadeOut(
                VGroup(

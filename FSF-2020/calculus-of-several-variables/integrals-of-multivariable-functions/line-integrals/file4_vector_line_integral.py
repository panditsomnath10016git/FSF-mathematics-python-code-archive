...
class LineIntegrationProcess(GraphScene):
    ...
    def construct(self):
        ...
        fn_text = TexMobject(
            r"\vec F = x^2\hat i-xy\hat j", stroke_width=2.5
        ).set_color_by_gradient(
            *self.default_vector_field_config["colors"]
        )
        fn_text.to_edge(TOP, buff=0.1).shift(2 * LEFT)
    ...
    def get_line_of_int(self):
        ...
        line_of_int_text.to_corner(UR, buff=0.8)
    ...
    def get_vector_and_tangent(self):
        ...
        vect_tangent_text.to_corner(UR, buff=0.8)
    ...
    def show_vector(self):
        ...
        vect_label = TextMobject(
            "$\\vec F(x_i,y_i)$", color=YELLOW, stroke_width=2
        ).scale(0.8)

        vector = always_redraw(
            lambda: self.vector_field.get_vector(
                self.coords_to_point(np.cos(t()), np.sin(t())),
                stroke_width=6,
                max_stroke_width_to_length_ratio=8,
            ).set_color(YELLOW),
        )

        vect_label.next_to(vector, RIGHT, buff=0.1)
    ...
    def show_tangent(self):
        tangent_label = TextMobject(
            "$\\vec T(x_i,y_i)$", color=ORANGE, stroke_width=2
        ).scale(0.8)
        ...
        tangent = always_redraw(
            ...
            .rotate(
                self.angle_of_tangent(
                    np.cos(t()), self.line_of_int, dx=-0.00001
                ),
        ...
        tangent_label.next_to(tangent, UP, buff=0.1)
    ...
    def write_about_graph(self):
        ...
        graph_text.to_corner(UR, buff=0.5)
    ...
    def show_the_parameter(self, t, t_axis):
        ...
        t_dot.add_updater(
            lambda obj: obj.move_to(self.c2g([t(), 0]))
        )
        t_text = TextMobject("$t$=").next_to(
            t_dot, UP, buff=0.25
        )
        t_val = always_redraw(
            lambda: DecimalNumber(t() / PI, color=GOLD)
            .next_to(t_text, RIGHT, buff=0)
            .scale(0.8)
        )
        t_label = VGroup(t_text, t_val).set_color(RED)
        ...
        t_label.add_updater(
            lambda label: label.next_to(t_dot, UP)
        )
    ...
    def show_graph_area(self, t_axis):
        t = self.t_tracker.get_value
        dot_prod_graph = always_redraw(
            lambda: Polygon(
                *[
                    self.c2g(
                        [t, -2 * np.cos(t) ** 2 * np.sin(t)]
                    )
                    for t in np.arange(0, t(), 0.01)
                ],
                *[self.c2g([t, 0]) for t in [t(), 0]],
                stroke_width=2.5,
                fill_color=TEAL_D,
                fill_opacity=0.6,
            )
    ...
    def write_area_as_intgral_value(self):
        ...
        self.play(
            ReplacementTransform(self.dot_prod_graph, area)
        )
        self.wait(0.5)

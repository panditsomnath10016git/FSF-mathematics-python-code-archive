...
class ParametricCurve(ThreeDScene):
    ...
    def construct(self):
        ...
        helix = self.get_helix(
            radius=1.5, t_min=0, t_max=4 * PI, color=PURPLE
        )
        parameter_label = TextMobject(
            "Parametric equation: ", color=TEAL
        ...
        parametric_eqn = TextMobject(
            ...
            r")\\$z$=",
            "t",
        ).next_to(parameter_label, DOWN, buff=0.1)
        parametric_eqn.set_color_by_tex("t", RED)
        self.parametric_eqn = parametric_eqn

        parametriztion = VGroup(parameter_label, parametric_eqn)
    ...
    def get_helix(self, radius=1, **kwargs):
        ...
        helix = ParametricFunction(
            lambda t: self.axes.c2p(
                radius * np.cos(t), radius * np.sin(t), t / 4
            ),
            **config
        )
    ...
    def show_the_parameter(self):
        ...
        t_label = TexMobject("t = ", color=RED).next_to(
            self.parametric_eqn, DL, buff=0.85
        )
        ...
        self.add(dot)
        self.play(
            t_tracker.set_value, 2, rate_func=linear, run_time=5
        )

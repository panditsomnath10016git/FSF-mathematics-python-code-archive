...
class VectorFields(ThreeDScene):
    ...
    def show_2d_field(self):
        d2_field_text = TexMobject(
            r"\vec F(x,y)=-y\hat i+x\hat j", stroke_width=1.5
        ).set_color_by_gradient(
            *self.default_vector_field_config["colors"]
        )
        d2_field_text.to_corner(UR, buff=0.5)

        d2_field = self.get_vector_field(
            lambda v: np.array([-v[1], v[0], 0]),
        )
    ...
    def show_3d_field(self):
        ...
        d3_field_text.to_corner(UR, buff=0.5)
...
class VectorField3D(VGroup):
    ...
    def __init__(self, func, **kwargs):
        ...
        z_range = np.arange(
            self.z_min, self.z_max + self.delta_z, self.delta_z
        )

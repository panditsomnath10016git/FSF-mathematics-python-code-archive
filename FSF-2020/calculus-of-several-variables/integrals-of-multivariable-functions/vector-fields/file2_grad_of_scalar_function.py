...
class GradOfScalarFunc(ThreeDScene):
    ...
    def construct(self):
        ...
        scalar_fn_text = TexMobject(
            "f(x,y)=", "\cos(xy)"
        ).set_color(BLUE)
        scalar_fn_text.to_corner(UR, buff=0.6)

        operator = (
            TexMobject("\\vec\\nabla")
            .next_to(scalar_fn_text, LEFT, buff=0.2)
            .set_color(GOLD)
        )

        grad_text = TexMobject(
            r"\dfrac{\partial f}{\partial x} \hat i+\dfrac{\partial f}{\partial y} \hat j+\dfrac{\partial f}{\partial z} \hat k"
        ).set_color(GOLD)
        grad_text.next_to(scalar_fn_text, DOWN).scale(0.9)

        VGroup(
            grad_text[0][1], grad_text[0][9], grad_text[0][17]
        ).set_color(BLUE)
        VGroup(
            grad_text[0][5:8],
            grad_text[0][13:16],
        ...
        s_field = self.get_scalar_field(
            func=lambda x, y: np.cos(x * y / 2), dn=0.25
        )
        v_field = self.get_vector_field(
            lambda v: np.array(
                [
                    -(v[1] - axes.c2p(0, 0, 0)[1])
                    * np.sin(
                        (v[0] - axes.c2p(0, 0, 0)[0])
                        * (v[1] - axes.c2p(0, 0, 0)[1])
                    ),
                    -(v[0] - axes.c2p(0, 0, 0)[0])
                    * np.sin(
    ...
    def get_scalar_field(self, func, dn=0.5):
    ...
    def get_points(self, func, dn):
        ...
        for x_val in x_vals:
            for y_val in y_vals:
                points += [
                    axes.c2p(
                        x_val, y_val, func(x_val, y_val) + 0.05
                    )
                ]
        return points

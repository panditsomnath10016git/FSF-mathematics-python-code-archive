...
class VectorFields(GraphScene):
    ...
    def show_func_machine(self):
        machine = RoundedRectangle(
            height=2, width=3.5, color=PURPLE, stroke_width=8
    ...
    def take_vec_from_machine(self, vect, position):
        ...
        if self.run_time > 0.5:
            point_coord = (
                TexMobject("(x_i,y_i)")
                .next_to(self.dot, RIGHT, buff=0.01)
                .scale(0.75)
            )
            ...
            self.play(FadeOut(input_point), run_time=0.2)
        ...
        self.play(vect.move_to, position, run_time=self.run_time)
    ...
    def scale_down_vectors(self, vector_function):
        ...
        for vector in self.non_scaled_vectors:
            scaled_vect = scaled_vector_field.get_vector(
                vector.get_center()
            )
        ...
        self.play(
            LaggedStartMap(
                MoveToTarget, self.non_scaled_vectors, run_time=3
            )
        )

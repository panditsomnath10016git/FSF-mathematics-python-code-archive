...
class SlopeFields(GraphScene):
    ...
    def construct(self):
        ...
        slope_field = self.get_slope_field(
            lambda x, y: -2.0
            * (x - self.graph_origin[0])
            * (y - self.graph_origin[1]),
    ...
    def show_func_machine(self):
        machine = RoundedRectangle(
            height=3, width=4, color=PURPLE, stroke_width=8
        ).to_edge(RIGHT, buff=0.4)

        machine_label = TextMobject(
            r"Line segment\\ with slope\\",
            " $y'=-2xy$",
            stroke_width=1.2,
            color=BLUE,
        ).next_to(machine, IN)
    ...
    def take_line_from_machine(self, vect, position):
        ...
        if self.run_time > 0.5:
            vect.next_to(self.func_machine, DOWN, buff=0.1)
            self.play(
                ApplyMethod(
                    self.dot.shift, DOWN, run_time=self.run_time
                )
            ),
            self.play(FadeOut(self.dot), run_time=0.2)
            self.play(FadeIn(vect), run_time=0.4)
            self.play(
                ApplyMethod(vect.move_to, position),
                run_time=self.run_time,
            )
        else:
...
class SlopeField(VGroup):
    ...
    def __init__(self, func, **kwargs):
        ...
        x_range = np.arange(
            self.x_min, self.x_max + self.delta_x, self.delta_x
        )
        y_range = np.arange(
            self.y_min, self.y_max + self.delta_y, self.delta_y
        )
    ...
    def get_slope(self, point, **kwargs):
        ...
        line = Line(
            ORIGIN,
            self.slope_length_factor * RIGHT,
            **line_config
        )
